...
Local Notation "X *" := (FinSeq X) (at level 1, left associativity, format "X *").

Section props.
  Universes u.
  Context {X:set@{u}} {z:Zero X}.
  ...
  Qed.
  Definition FinSeq_cons : X × X* ⇾ X* := @func_make _ _ _ FinSeq_cons_is_fun.
...
Section dec.
  Universes u.
  Context {X:set@{u}} {z:Zero X}.
  Local Abbreviation ϕ := FinSeq_sequence.
  ...
  Proof. intros [x y]; revert x y; refine (FinSeq_sinduction _ _ _).
  ...
  Definition FinSeq_zero_dec {d:Dec (A:=tprod X X) (=)} : X* → bool :=
  ...
  Definition FinSeq_eq_dec_raw {d:Dec (A:=tprod X X) (=)} : X* → X* → bool := fix F s t :=
    match s with
  ...
  Instance FinSeq_eq_dec {d:Dec (A:=tprod X X) (=)} : Dec (A:=tprod X* X*) (=) := tuncurry FinSeq_eq_dec_raw.
...
Global Hint Extern 2 (Dec (A:=tprod (set_T _*) _) (=)) => refine FinSeq_eq_dec : typeclass_instances.
...
Section tail.
  ...
  + intros x s. refine (ProdList_sdestruct _ _ _).
    ...
    * intros y t. change ((∏ i, [ ProdList_unit x ∙ s ]_i = [ ProdList_unit y ∙ t ]_i) ⊸ (∏ i, [s]_i = [t]_i)).
      rew <-all_adj. intros i. exact (all_lb _ (nat_S i)).
  Qed.
  Definition FinSeq_tail `{zX:Zero X} : _ ⇾ _ := @func_make _ _ _ (@tail_is_fun X _).
...
Require Import rewrite_preserves.

Section map.
  Universes u.
  Context {X Y:set@{u}} (f:X ⇾ Y) {zX:Zero X} {zY:Zero Y} {H:Zero_Pointed_Morphism f}.

  Local Abbreviation ϕ := (ProdList_index_or_zero _).
  ...
  Proof. refine (ProdList_sinduction_alt _ _ _); [ now intro | intros x₀ x IHx ].
    rewrite_preserves (ProdList_map f). intros [| i]; [ refl | exact (IHx _) ].
  Qed.

  Local Instance: @IsFun X* Y* (ProdList_map f).
  Proof. intros s t. change (s = t ⊸ ϕ (ProdList_map f s) = ϕ (ProdList_map f t)).
    rew (FinSeq_map_spec' _). exact (is_fun (f ∘) _ _).
  Qed.

  Definition FinSeq_map : _ ⇾ _ := @func_make X* Y* (ProdList_map f) _.
...
Section map2.
  ...
  Universes u.
  Context {X Y Z: set@{u}} {zX : Zero X} {zY : Zero Y} {zZ : Zero Z}.
  ...
  := set:(λ '(x, l, g) : X × ProdList X × (ProdList Y ⇾ ProdList Z),
    ProdList_match (X:=Y) (
      ProdCons (f (x, 0), map2_zero_r l) ,
      set:(λ '(y, l') : Y × ProdList Y,  ProdCons (f (x, y), g l') )
    )).
  ...
  Proof. intros [s₁ t₁][s₂ t₂]. change (s₁ = s₂ ⊠ t₁ = t₂ ⊸ ∏ n, ϕ (map2 s₁ t₁) n = ϕ (map2 s₂ t₂) n).
    rew <-all_adj. intros n. rew (map2_spec' _ _ _).
    rew <-(is_fun f _ _); unfold_pair_eq.
    refine (aprod_proper_aimpl _ _); exact (all_lb _ _).
  Qed.

  Definition FinSeq_map2 : _ ⇾ _ := @func_make (X* ⊗ Y*) Z* (uncurry map2) _.

  Definition FinSeq_map2_spec : ∀ s t n, FinSeq_index n (FinSeq_map2 (s, t)) = f (FinSeq_index n s, FinSeq_index n t) := map2_spec'.
End map2.
...
Section add_mon.
  ...
  + intros s t. now rew [ (FinSeq_tail_spec s : apos _) | (FinSeq_tail_spec t : apos _) ].
  + refl.
  Qed.
End add_mon.

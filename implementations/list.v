...
Definition tensor_to_prod_list {X} := @func_make _ _ _ (@tensor_to_prod_list_is_fun X).
...
Proof. refine (ProdList_sinduction_alt _ _ _); [ easy | intros a x IHx ].
  ...
  change (a = b ∧ x = y ⊸ a = b ⊠ id x = id y :> TensorList X).
  rew <-(IHx y). now apply aand_aprod_dec_l.
Qed.

Definition dec_prod_to_tensor_list {X} `{!DecidableEquality X}  := @func_make _ _ _ (@dec_prod_to_tensor_list_is_fun X _).

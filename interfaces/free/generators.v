...
Class Var@{u} (X:set@{u}) := var : Nat@{u} → X.
Arguments var {X _} _%_nat.

(** [ Var_Morphism Γ f ] means [ f ] sends [ var n ] to the [n]th element of [ Γ ] *)
Class Var_Morphism@{u} {X Y:set@{u}} {v:Var X} (Γ:list Y) (f:X ⇾ Y) : SProp
  := preserves_var (n:Nat) {b:ListInBounds Γ n} : f (var n) = list_nth Γ n.
Arguments preserves_var {X Y _} Γ f {_} n%_nat {_}.
...
Record var_set :=
{ var_set_car :> set
; #[canonical=no, reversible=no] var_set_var :> has_var var_set_car
}.
...
Record var_morphism_t@{u} (X:var_set@{u}) {Y:set@{u}} (Γ:list Y) :=
{ var_morphism_func :> func X Y
; #[canonical=no, reversible=no] var_morphism_prop :> Var_Morphism Γ var_morphism_func
}.
...
Canonical Structure var_morphism@{u} (X:var_set@{u}) {Y:set@{u}} (Γ:list Y)
  := projected_set (@var_morphism_func X Y Γ).
Definition make_var_morphism@{u} : ∀ {X:var_set@{u}} {Y:set@{u}} (Γ:list Y) (f:X ⇾ Y),

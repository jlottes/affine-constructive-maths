...
Section definition.
  Universes u.
  Context `{AdditiveMonoid@{u} M} `{AdditiveGroup@{u} K}.

  Class FromGroupCompletion (i:M ⇾ K) := from_group_completion
    : ∀ {A:additive_non_com_group@{u}} (f:M ⇾ A) `{!AdditiveMonoid_Morphism f}, (K ⇾ A).
  Global Arguments from_group_completion i {_ A} f {_}.

  Class FromGroupCompletionProp (i:M ⇾ K)
    {A:additive_non_com_group@{u}} (f : M ⇾ A) (g : K ⇾ A) : SProp :=
  ...
  ; from_group_completion_prop {A:additive_non_com_group@{u}} (f : M ⇾ A) `{!AdditiveMonoid_Morphism f}

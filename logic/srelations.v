...
Lemma flip_sSubrelation   `{!@sSubrelation   (A∗A) R R'} : sSubrelation (sflip R) (sflip R').  Proof. firstorder. Qed.
...
Global Hint Extern 1 (sSubrelation ?R ?R) => simple notypeclasses refine (sSubrelation_refl_applied _) : typeclass_instances.

(*
Lemma sSubrelation_refl    {A} : sReflexive  (@sSubrelation A).     Proof sSubrelation_refl_applied.
Lemma sSubrelation_trans   {A} : sTransitive (@sSubrelation A).     Proof. intros ??? H1 H2 ?? ?; now apply H2, H1. Qed.
...
Lemma impl_proper_impl     {P₁ P₂ Q₁ Q₂}: impl (P₂, P₁) → impl (Q₁, Q₂) → impl (impl (P₁, Q₁), impl (P₂, Q₂)).  Proof. exact (sTransitive_rel_proper_impl impl). Qed.
Lemma impl_proper_iff  : ∀ {P₁ P₂ Q₁ Q₂}, iff  (P₁, P₂) → iff  (Q₁, Q₂) → iff  (impl (P₁, Q₁), impl (P₂, Q₂)).  Proof. firstorder. Qed.
Lemma iff_proper_impl      {P₁ P₂ Q₁ Q₂}: iff  (P₁, P₂) → iff  (Q₁, Q₂) → impl (iff  (P₁, Q₁), iff  (P₂, Q₂)).  Proof. exact (sPER_rel_proper_impl iff). Qed.
Lemma iff_proper_iff   : ∀ {P₁ P₂ Q₁ Q₂}, iff  (P₁, P₂) → iff  (Q₁, Q₂) → iff  (iff  (P₁, Q₁), iff  (P₂, Q₂)).  Proof. firstorder. Qed.
Lemma not_proper_impl      {P₁ P₂}      : impl (P₂, P₁) → impl (not P₁, not P₂).  Proof. unfold impl. tauto. Qed.
Lemma not_proper_iff   : ∀ {P₁ P₂}      , iff  (P₁, P₂) → iff  (not P₁, not P₂).  Proof. firstorder. Qed.
Lemma or_proper_impl       {P₁ P₂ Q₁ Q₂}: impl (P₁, P₂) → impl (Q₁, Q₂) → impl (or  P₁ Q₁, or  P₂ Q₂).  Proof. unfold impl. tauto. Qed.
Lemma or_proper_iff    : ∀ {P₁ P₂ Q₁ Q₂}, iff  (P₁, P₂) → iff  (Q₁, Q₂) → iff  (or  P₁ Q₁, or  P₂ Q₂).  Proof. firstorder. Qed.
Lemma and_proper_impl      {P₁ P₂ Q₁ Q₂}: impl (P₁, P₂) → impl (Q₁, Q₂) → impl (and P₁ Q₁, and P₂ Q₂).  Proof. unfold impl. tauto. Qed.
Lemma and_proper_iff   : ∀ {P₁ P₂ Q₁ Q₂}, iff  (P₁, P₂) → iff  (Q₁, Q₂) → iff  (and P₁ Q₁, and P₂ Q₂).  Proof. firstorder. Qed.
...
Global Hint Extern 2 (impl (impl _, _) ) => sapply_2 impl_proper_impl : proper.
Global Hint Extern 2 (iff  (impl _, _) ) => sapply_2 impl_proper_iff  : proper.
Global Hint Extern 2 (impl (iff  _, _) ) => sapply_2 iff_proper_impl  : proper.
Global Hint Extern 2 (iff  (iff  _, _) ) => sapply_2 iff_proper_iff   : proper.
...
Ltac ex_proper_impl_tac :=
  refine ((λ H : (∀ x, impl _), λ '(exists _ x p), exists _ x (H x p)) (λ _, _)).

Ltac ex_proper_iff_tac :=
  refine ((λ H : (∀ x, iff _), conj (λ '(exists _ x p), exists _ x (andl (H x) p))

Require Import interfaces.notation sprop tactics.easy srelations tactics.misc rewrite.proper.
...
Local Ltac handle_subrel R2 R :=
  lazymatch R2 with
  | R => exact (λ E, E)
  | _ => solve [ let S := get_instance (sSubrelation R2 R) in simple refine (S _)
               | let g := get_goal in
...
Local Ltac fixup_given_sym _ :=
  ...
  | |- ?P → ?R (?a, ?b) =>
    echange (_ (b, a) → R (a, b));
    lazymatch goal with |- ?R2 _ → _ => handle_symmetry R2 R a b end
  end.

Local Ltac fixup_given _ :=
  lazymatch goal with
  | |- ?R2 ?p → ?R ?p => handle_subrel R2 R
  | |- ?P → ?R ?p =>
    echange (_ p → R p);
    lazymatch goal with |- ?R2 _ → _ => handle_subrel R2 R end
  end.
...
(* Linear [(?a, ?b)] + universe-insensitive [lazymatch a with b] guard; see
   the same idiom in arewrite.v. *)

...
Proof. hnf; intros x y z. rew (commutativity f _ z). exact (distribute_l f g _ _ _). Qed.

Lemma left_distr_from_right `{!@Commutative X X f, !RightDistribute f g} : LeftDistribute f g.
Proof. hnf; intros x y z. rew (commutativity f x _). exact (distribute_r f g _ _ _). Qed.

Definition involutive_alt `(f:X ⇾ X) `{!Involutive f} x : f (f x) = x := involutive f x.
Definition idempotent_alt `(f:X ⇾ X) `{!Idempotent f} x : f (f x) = f x := idempotent f x.

Lemma involutive_bijective `{Involutive (f:=f)} : Bijective f (inv:=f).
Proof. split; hnf.
+ intros x y. rew <-(involutive_alt f x) at 2. rew <-(involutive_alt f y) at 2. exact (is_fun f _ _).
+ exact (involutive f).
Qed.
...
Proof. exact (acontra (left_or_right_absorb f x y)). Qed.
...
Lemma absorb_ne_left_prod_right {X:set} `{!RefutativeEquality X} (f:X ⊗ X ⇾ X) `{!LeftAbsorb f z, !RightAbsorb f z} (x y : X)
  : f (x, y) ≠ z ⊸ x ≠ z ⊠ y ≠ z.
Proof. exact (acontra (left_par_right_absorb f x y)). Qed.

Section cancellation.
  Universes u.
  Context {X Y Z : set@{u}} (f:X ⊗ Y ⇾ Z).
...
Proof. intros P z x y. change (f(x, z) = f(y, z) ⊸ x = y). rew (commutativity f _ z). apply P. Qed.
...
Section opposite.
  ...
  Lemma Distribute_op     `{H:Distribute     (f:=f) (g:=g)} : Distribute     f (g ∘ tensor_swap _ _).  Proof. go H. Qed.
  Lemma AntiDistribute_op `{H:AntiDistribute (f:=f) (g:=g)} : AntiDistribute f (g ∘ tensor_swap _ _).  Proof. go H. Qed.
End opposite.
...
Global Hint Extern 2 (Distribute     _ (_ ∘ tensor_swap _ _)    ) => notypeclasses refine Distribute_op       : typeclass_instances.
Global Hint Extern 2 (AntiDistribute _ (_ ∘ tensor_swap _ _)    ) => notypeclasses refine AntiDistribute_op   : typeclass_instances.

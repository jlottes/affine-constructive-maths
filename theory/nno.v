...
Require Import quote.base rewrite_preserves change_quantifiers simplify.

Import modality_notation.

Section quote.
  Universes u.
  Context {X Y:set@{u}} (f:X ⇾ Y) `{MaybeAlgebra_Morphism@{u} (X:=X) (Y:=Y) (f:=f)}.
...
Lemma alt_Build_MaybeAlgebra_Morphism@{u} {X Y:set@{u}} `{Zero X} `{Successor X} `{Zero Y} `{Successor Y} (f:X ⇾ Y) :
...
Lemma compose_maybe_algebra_morphism@{u} {X₁ X₂ X₃:set@{u}}
  `{Zero X₁} `{Successor X₁}
  `{Zero X₂} `{Successor X₂}
  `{Zero X₃} `{Successor X₃}
  {f:X₁ ⇾ X₂} {g:X₂ ⇾ X₃}
...
+ intro y. quote_injective f. now rew (surjective_applied f _).
Qed.
...
Lemma nno_initial_alt@{u} `{NaturalNumbersObject@{u} N}
  {X:set@{u}} `{Zero X} `{Successor X} (f g : N ⇾ X)
...
Section contents.
  ...
  Let ψ := (nno_to_set@{u} ℕ Nat).
  Local Hint Extern 2 => progress unfold ψ : typeclass_instances.

  Let inst : MaybeAlgebra_Morphism ψ.  Proof. exact _. Qed.
  Let inst2 : Bijective ψ.  Proof. exact _. Qed.
  Let inst3 : MaybeAlgebra_Morphism ψ⁻¹.  Proof. exact _. Qed.

  Instance nno_suc_inj : Injective (X:=ℕ) suc.
  Proof. intros x y. rew (injective_iff ψ _ _). rewrite_preserves ψ.
  ...
  Let inst4 : DecidableEquality ℕ.  Proof. exact nno_dec_eq. Qed.
  ...
  Proof. intros Pp P0 Ps.
    simple notypeclasses refine (let P' := func_make (λ n, ! (P n)) in _).
  ...
  Abbreviation ϕ f x := (@nno_to_set@{u} ℕ _ X x f).
  ...
  Proof.
    ...
    simple refine (let P := func_make (λ n, ϕ f x₁ n = ϕ f x₂ n) in nno_ind P _).
    + intros n m. apply affirmative_aimpl. intro E. change ((ϕ f x₁ n = ϕ f x₂ n) ⧟ (ϕ f x₁ m = ϕ f x₂ m)). now rew E.
    + intros n. change (ϕ f x₁ n = ϕ f x₂ n ⊸ ϕ f x₁ (suc n) = ϕ f x₂ (suc n)); clear P.
      rew [ (step f x₁ n) | (step f x₂ n) ]. exact (is_fun f _ _).
  Qed.

  Definition nno_rec (f:X ⇾ X) : X × ℕ ⇾ X.
  Proof. simple notypeclasses refine (strong_op (uncurry _)).
  + exact (func_make (λ x : X, ϕ f x)).
  + refine (dec_strong_op_r _).
  Defined.
  ...
  Proof. intros f g.
    ...
    + rew <-all_adj. intros x. rew [ (base f x) | (base g x) ]; now simplify.
  ...
  Definition nno_rec_fun := func_make nno_rec.

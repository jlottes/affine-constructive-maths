...
Section props.
  Universes u.
  Context {X Y Z D:set@{u}}.
  
  Lemma pointwise_associative       {f} {Hf:PointwiseOp f D}                      : @Associative      X     f     → Associative      (pointwise_op   f D).  Proof. doit. Qed.
  Lemma pointwise_commutative       {f} {Hf:PointwiseOp f D}                      : @Commutative      X Y   f     → Commutative      (pointwise_op   f D).  Proof. doit. Qed.
  Lemma pointwise_binary_idempotent {f} {Hf:PointwiseOp f D}                      : @BinaryIdempotent X     f     → BinaryIdempotent (pointwise_op   f D).  Proof. doit. Qed.
  (*Lemma pointwise_involutive        {X     f     D}                                           : @Involutive       X     f     → Involutive       (pointwise_func f D).  Proof. doit. Qed.*)
  Lemma pointwise_absorption        {f g} {Hf:PointwiseOp f D} {Hg:PointwiseOp g D} : @Absorption       X Y Z f g   → Absorption       (pointwise_op   f D) (pointwise_op g D).  Proof. doit. Qed.
  Lemma pointwise_left_distribute   {f g} {Hf:PointwiseOp f D} {Hg:PointwiseOp g D} : @LeftDistribute   X     f g   → LeftDistribute   (pointwise_op   f D) (pointwise_op g D).  Proof. doit. Qed.
  Lemma pointwise_right_distribute  {f g} {Hf:PointwiseOp f D} {Hg:PointwiseOp g D} : @RightDistribute  X     f g   → RightDistribute  (pointwise_op   f D) (pointwise_op g D).  Proof. doit. Qed.
  
  Lemma pointwise_left_identity     {f x} {Hf:PointwiseOp f D}                      : @LeftIdentity     X Y   f x   → LeftIdentity     (pointwise_op   f D) (const x).  Proof. doit. Qed.
  Lemma pointwise_right_identity    {f y} {Hf:PointwiseOp f D}                      : @RightIdentity    X Y   f y   → RightIdentity    (pointwise_op   f D) (const y).  Proof. doit. Qed.
  Lemma pointwise_left_absorb       {f x} {Hf:PointwiseOp f D}                      : @LeftAbsorb       X Y   f x   → LeftAbsorb       (pointwise_op   f D) (const x).  Proof. doit. Qed.
  Lemma pointwise_right_absorb      {f y} {Hf:PointwiseOp f D}                      : @RightAbsorb      X Y   f y   → RightAbsorb      (pointwise_op   f D) (const y).  Proof. doit. Qed.
  
  Lemma pointwise_left_inverse      {f g x} {Hf:PointwiseOp f D}                      : @LeftInverse      X Y Z f g x → LeftInverse      (pointwise_op   f D) (pointwise_func g D) (const x).  Proof. doit. Qed.
  Lemma pointwise_right_inverse     {f g x} {Hf:PointwiseOp f D}                      : @RightInverse     X Y Z f g x → RightInverse     (pointwise_op   f D) (pointwise_func g D) (const x).  Proof. doit. Qed.
End props.
...
Section structures.
  ...
  Local Instance pointwise_meet_sl         `{Meet X} `{!PointwiseOp (X:=X) meet D}             : MeetSemiLattice        X → MeetSemiLattice        (D ⇾ X).  Proof. exact pointwise_semilattice. Qed.
  Local Instance pointwise_bounded_meet_sl `{Meet X} `{!PointwiseOp (X:=X) meet D} `{Top X}    : BoundedMeetSemiLattice X → BoundedMeetSemiLattice (D ⇾ X).  Proof. exact pointwise_bounded_sl. Qed.
  Local Instance pointwise_join_sl         `{Join X} `{!PointwiseOp (X:=X) join D}             : JoinSemiLattice        X → JoinSemiLattice        (D ⇾ X).  Proof. exact pointwise_semilattice. Qed.
  Local Instance pointwise_bounded_join_sl `{Join X} `{!PointwiseOp (X:=X) join D} `{Bottom X} : BoundedJoinSemiLattice X → BoundedJoinSemiLattice (D ⇾ X).  Proof. exact pointwise_bounded_sl. Qed.
  ...
  Local Instance pointwise_add_nc_mon `{Plus X} `{!PointwiseOp (X:=X) plus D} `{Zero X}             : AdditiveNonComMonoid X → AdditiveNonComMonoid (D ⇾ X).  Proof. exact pointwise_monoid. Qed.
  Local Instance pointwise_add_mon    `{Plus X} `{!PointwiseOp (X:=X) plus D} `{Zero X}             : AdditiveMonoid       X → AdditiveMonoid       (D ⇾ X).  Proof. exact pointwise_com_monoid. Qed.
  Local Instance pointwise_add_nc_grp `{Plus X} `{!PointwiseOp (X:=X) plus D} `{Zero X} `{Negate X} : AdditiveNonComGroup  X → AdditiveNonComGroup  (D ⇾ X).  Proof. exact pointwise_group. Qed.
  Local Instance pointwise_add_grp    `{Plus X} `{!PointwiseOp (X:=X) plus D} `{Zero X} `{Negate X} : AdditiveGroup        X → AdditiveGroup        (D ⇾ X).  Proof. exact pointwise_abgroup. Qed.

  Local Instance pointwise_mult_sg      `{Mult X} `{!PointwiseOp (X:=X) mult D}          : MultiplicativeSemiGroup X → MultiplicativeSemiGroup (D ⇾ X).  Proof. exact pointwise_semigroup. Qed.
  Local Instance pointwise_mult_mon     `{Mult X} `{!PointwiseOp (X:=X) mult D} `{One X} : MultiplicativeMonoid    X → MultiplicativeMonoid    (D ⇾ X).  Proof. exact pointwise_monoid. Qed.
  Local Instance pointwise_mult_com_mon `{Mult X} `{!PointwiseOp (X:=X) mult D} `{One X} : MultiplicativeComMonoid X → MultiplicativeComMonoid (D ⇾ X).  Proof. exact pointwise_com_monoid. Qed.
End structures.
